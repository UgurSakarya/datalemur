...
FROM measurements )
select 
cast(rn.measurement_time as date) as measurement_day,
sum(case when rn.rn %2 != 0 then rn.measurement_value else 0 end) as odd_sum,
sum(case when rn.rn %2 = 0 then rn.measurement_value else 0 end) as even_sum
from rn

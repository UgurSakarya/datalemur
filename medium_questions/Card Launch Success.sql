...
SELECT 
card_name,
to_date(issue_year || '-' || issue_month, 'YYYY-MM' ) as dates,
min(to_date(issue_year || '-' || issue_month, 'YYYY-MM' )) over(partition by card_name) as publish_date,
issued_amount
FROM monthly_cards_issued
) A
where dates = publish_date

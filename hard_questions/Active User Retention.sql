...
where EXISTS (select user_id from user_actions prev where curr.user_id = prev.user_id and extract(month from curr.event_Date - interval '1 Month')= extract(month from prev.event_date))
